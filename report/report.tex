...
\subsection{Interest Points and Descriptors}
 ...
 After some tests, the SIFT implementations showed some good keypoints detection. In Fig. \ref{fig:keypoints} it shows the keypoints detected and even though it didn catch as many as OpenCV's sift, it looked well enough. So, the descriptors get changed to the format expected by the matching part. The keypoints got converted to KeyPoint object from OpenCV and the features fot concatenated instead of splited by octave layer. The matcher would be able to find the matches and the threshold of 5 min matches would me satisfied.
 ...
 More test was made with the extractor, but apparently the project's SIFT, although it extract some relevant keyponts, the information around them showed themselves too poor, so no progress was made that could be compared to OpenCV's SIFT.

 The Fig. \ref{fig:keypoints_arrow}
\begin{figure}[H]
     \centering
     \includegraphics[width=0.4\hsize]{img/o-kp-0.png}
     \includegraphics[width=0.4\hsize]{img/o-kp-1.png}
